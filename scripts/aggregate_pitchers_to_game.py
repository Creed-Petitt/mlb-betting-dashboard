...
# === Add strikeout and innings-pitched targets ===
df['is_pitcher_strikeout'] = df['events'].isin(['strikeout', 'strikeout_double_play']).astype(int)
df['out_made'] = df['events'].isin(['strikeout', 'field_out', 'force_out', 'grounded_into_double_play', 'double_play', 'triple_play']).astype(int)
...
print(f"\nSaved pitcher_game_stats table with {len(agg)} rows (now includes innings pitched)")
